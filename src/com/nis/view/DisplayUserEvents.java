		...
		if(rs.next()){
			...
			do{
				out.println("<tr><td>"+sn+"</td>"
				    	...
				    	+ "<td><a class='btn btn-info btn-xs' href='TicketPrinting?tid="+rs.getString(1)+"'>Generate</a><br><a class='btn btn-success btn-xs' href='https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=200x200&chld=M|0&cht=qr&chl=http://192.168.43.153:88/VIS/TicketPrinting?tid="+rs.getString(1)+"'>On Mobile</td></tr>");
				sn++;
			}while(rs.next());
		}
		else{
